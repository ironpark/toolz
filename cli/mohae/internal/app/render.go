...
func renderTerminal(results []TrialResult, options ReportOptions) string {
	out := &strings.Builder{}
	for _, result := range results {
...
func renderJSON(results []TrialResult, options ReportOptions) (string, error) {
...
func renderMarkdown(results []TrialResult, options ReportOptions) string {
	out := &strings.Builder{}
...
// arrive unreadable.
func renderHTML(results []TrialResult, options ReportOptions) string {
	out := &strings.Builder{}
...
// down by category.
func usageText(usage TokenUsage, detailed bool) string {
	if !detailed {
