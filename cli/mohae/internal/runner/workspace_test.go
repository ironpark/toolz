...
func fixtureConfig(t *testing.T, directory string) *Config {
	...
	config := &Config{
		...
		Limits:    configuration.LimitsConfig{TimeoutSeconds: configuration.DefaultTimeoutSeconds},
		Report:    configuration.ReportConfig{Dir: configuration.DefaultReportDir, Formats: []string{"terminal"}},
	}
	return config
}
...
func TestPrepareWorkspaceInstallsOnlyTheSkillsScopedToTheAgent(t *testing.T) {
	...
	defer workspace.Cleanup()
	skillDir, _ := agent.SkillDir("custom-cli")
