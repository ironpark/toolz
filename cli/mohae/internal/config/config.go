...
func (c *Config) Validate() error {
	if c.Agent.Type == "" {
		return fmt.Errorf("agent.type is required (one of: %s)", strings.Join(agent.KnownTypes, ", "))
	}
	if !agent.IsKnown(c.Agent.Type) {
		return fmt.Errorf("unknown agent.type %q (one of: %s)", c.Agent.Type, strings.Join(agent.KnownTypes, ", "))
	}
...
func validateAgents(field string, agents []string) error {
	for _, name := range agents {
		if !agent.IsKnown(name) {
			return fmt.Errorf("%s.agents: unknown agent type %q (one of: %s)", field, name, strings.Join(agent.KnownTypes, ", "))
		}
	}
	return nil
}
...
func validateWorkspacePattern(pattern string) error {
	...
	for _, segment := range workspacePatternSegments(pattern) {
		if segment == ".." {
...
func workspacePatternSegments(pattern string) []string {
