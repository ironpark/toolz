...
3. **실행** — `prompts`를 순서대로 보냅니다. 각 프롬프트는 앞 턴이 끝난 뒤에 전송되고,
   ...
   기록합니다.
4. **완료 훅** — `hooks.after`가 에이전트 종료 직후 선택한 scope에서 후처리를 실행합니다.
5. **채점** — `verify.commands`가 순서대로 끝난 워크스페이스를 검사합니다. 워크스페이스 **밖**에서
   실행되고 안으로 복사되지도 않으므로, 에이전트가 검사 항목에 맞춰 결과를 꾸밀 수
   없습니다.
6. **리포트** — 성공 여부, 토큰 종류별 사용량, 소요 시간, 실패한 명령을 리포트로 남깁니다.
...
hooks:
  after: # 문자열은 scope: workspace의 축약형
    - ./finalize.sh
...
보냅니다. 에이전트가 멈추면 `hooks.after` 명령을 지정한 위치에서 순서대로 실행한 뒤,
워크스페이스 **바깥**에서 `verify.commands`를 실행합니다. 원본은
읽기만 하므로 같은 설정을 두 번 돌리면 항상 같은 상태에서 시작합니다.

완료 훅은 에이전트가 오류나 시간 초과로 멈춰도 실행되며, 만료된 trial과 분리된 새 제한
시간을 받습니다. 에이전트와 같은 `MOHAE_WORKSPACE`, `MOHAE_TRIAL`, `MOHAE_MODEL` 등의
환경 변수를 봅니다. `scope`의 기본값인 `workspace`는 현재 디렉터리를
`$MOHAE_WORKSPACE`로 정해 후처리 결과가 검증과 artifact 수집에 포함되게 합니다. `outside`는
verify와 같은 격리된 scratch 디렉터리에서 실행하므로 훅이 만든 파일이 워크스페이스 결과로
섞이지 않습니다. 어느 위치에서든 `$MOHAE_WORKSPACE`로 완성된 작업물을 읽을 수 있습니다.
하나라도 0이 아닌 종료 코드로 끝나면 trial은
실패하지만, 나머지 훅과 검증은 진단을 위해 계속 실행되고 출력과 종료 코드는 리포트에
기록됩니다.
