...
# sandbox: # container 대신 쓰는 가벼운 격리 (macOS). 둘은 병행할 수 없습니다
#   enabled: true
#   scope: full # setup(기본) | full — full은 에이전트까지 묶음
#   allow_write: [~/.cache/go-build] # 기본 허용 외에 더 열어 줄 경로
...
`sandbox`는 trial을 이 머신에 둔 채로 **쓰기만** 워크스페이스 안으로 묶습니다. 컨테이너와
목적이 다르므로 섹션도 따로입니다.

- **컨테이너**는 툴체인을 고정합니다. 어떤 컴파일러와 어떤 테스트 러너로 빌드·채점하는지를
  정하므로, 다른 머신에서도 같은 측정이 됩니다.
- **샌드박스**는 아무것도 고정하지 않습니다. 이 머신에 깔린 것을 그대로 쓰고, 어디에 쓸 수
  있는지만 제한합니다.

|                | 툴체인 고정 | 쓰기 차단 | 비용            | 요구사항          |
| -------------- | ----------- | --------- | --------------- | ----------------- |
| 없음           | ✗           | ✗         | 0               | —                 |
| `sandbox`      | ✗           | ✓         | 실행당 ~3ms     | macOS             |
| `container`    | ✓           | ✓         | trial당 ~2.5s   | docker 또는 podman |

즉 **한 머신에서 에이전트끼리 공정하게 비교**할 때는 샌드박스로 충분하고, **다른 머신과
비교**하려면 여전히 컨테이너가 필요합니다. 리포트의 `sandbox` 필드에 어느 scope로
돌았는지가 남습니다.

`scope: full`은 [알려진 한계](#알려진-한계)의 `claude-code` 문제를 컨테이너보다 훨씬 싸게
해소합니다 — 이미지에 에이전트 CLI를 넣을 필요도, 로그인 자격 증명을 마운트할 필요도
없습니다.

동작에 관해 알아 둘 것:

- **읽기는 막지 않습니다.** 호스트에 설치된 CLI와 라이브러리를 그대로 쓰는 것이 목적이라,
  컴파일러가 무엇을 읽어야 하는지 추측하는 대신 읽기는 전부 허용합니다. 가려야 할 경로가
  있으면 `deny_read`로 지정합니다.
- **기본 쓰기 허용.** 워크스페이스, scratch, trial의 `home`, trial 전용 임시 디렉터리, 그리고
  홈 아래 `.cache` `.config` `.local` `.npm` `.claude` `.codex`입니다. 마지막 목록은 에이전트
  CLI의 세션·캐시와 패키지 매니저가 쓰는 곳이라 막으면 격리가 아니라 실행 불가가 됩니다.
  결과물을 남길 만한 곳은 아닙니다.
- **`$TMPDIR`.** trial 전용 디렉터리로 바꿔 줍니다. macOS의 공용 임시 디렉터리는
  `/var/folders/...`이고 **워크스페이스가 그 안에 있어서**, 그것을 열어 주면 샌드박스가
  통째로 무의미해지기 때문입니다.
- **`/tmp`는 열려 있지 않습니다.** 필요하면 `allow_write`에 적으십시오.
- **프로파일 위치.** 생성된 seatbelt 프로파일은 trial 디렉터리 최상단에 두며, 그곳은 쓰기가
  허용되지 않습니다. trial이 고쳐 쓸 수 있는 프로파일은 첫 명령만 묶기 때문입니다.
- **경로 해석.** 규칙은 심볼릭 링크를 푼 경로로 씁니다. macOS가 `/tmp`로 보여 주는 것을
  샌드박스는 `/private/tmp`로 대조하므로, 그러지 않으면 규칙이 조용히 아무것도 매칭하지
  못합니다.
- **플랫폼.** 현재 macOS 전용입니다. 다른 OS에서는 설정 로드 단계에서 실패합니다 —
  격리되지 않은 실행이 격리된 것으로 기록되는 편이 더 나쁘기 때문입니다.
...
| `--sandbox`              | 컨테이너 없이 쓰기를 워크스페이스로 묶음 (macOS)           |
| `--sandbox-scope`        | `setup`(기본) 또는 `full`                                  |
...
에이전트 모두 컨테이너가 경계입니다. macOS에서는
[`sandbox.scope: full`](#가벼운-샌드박스-macos)이 같은 것을 훨씬 싸게 해 줍니다:
이미지도, 자격 증명 마운트도 필요 없습니다.
