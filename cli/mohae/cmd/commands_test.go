...
func run(t *testing.T, arguments ...string) error {
	t.Helper()
	return New(Options{Version: "test"}).Run(context.Background(), append([]string{"mohae"}, arguments...))
}
...
func TestEveryCommandIsReachable(t *testing.T) {
	...
	for _, command := range New(Options{Version: "test"}).Commands {
...
func TestRunAppliesProfilesBeforeFlagOverrides(t *testing.T) {
	directory := chdir(t)
	contents := minimalConfig + `profiles:
  claude:
    agent:
      type: claude-code
      model: claude-opus-5
`
	...
	command := flagsOnly(runCommandDefinition())
	if err := command.Run(t.Context(), []string{"run", "--profile", "claude", "--timeout", "42"}); err != nil {
		t.Fatal(err)
	}
...
func TestRunPromptConditionsAttachByPosition(t *testing.T) {
	command := flagsOnly(runCommandDefinition())
	arguments := []string{
		"run", "--prompt", "first", "--prompt", "second",
		"--prompt-when", "", "--prompt-when", "turn == 2",
	}
	...
	if len(prompts) != 2 || prompts[0].When != "" || prompts[1].When != "turn == 2" {
