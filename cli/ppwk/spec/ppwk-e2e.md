...
**현재 지원하지 않는다.** go-git v6 가 `refstorage` 확장을 모르기 때문에 저장소를 열지조차 못한다. 읽기를 전부 `git` 실행으로 바꾸지 않는 한 고칠 수 없고, 그것은 §14.1 의 선택을 되돌리는 일이다.

```
git init --ref-format=reftable 로 생성 후 init
```

검증:

```
무엇이 안 되는지와 무엇을 하면 되는지를 밝히며 거부
    ("reftable ref backend 는 아직 지원하지 않습니다 ... --ref-format=files")
```

지원이 생기면 이 시나리오가 실패한다. 그때 전체 워크플로우 검증으로 바꾼다.
...
| 티어 | 시나리오 | 실행 시점 | 구현 |
|---|---|---|---|
| smoke | E2E-1, 2, 3 | 모든 커밋 | `go test ./e2e/ -short` |
| core | 1~24 (17b~17d, 22b~22g 포함) | 모든 PR | `go test ./e2e/` |
| matrix | 1~28 × 전 조합 | 야간 | 아래 환경변수 |
| soak | 29, 30 | 릴리스 전, 주 1회 | `PPWK_E2E_SOAK=24h` |

반복 횟수와 규모는 환경변수로 연다. 기본값은 PR 에서 몇 분 안에 끝나도록 낮추되, 실패가 한 번이라도 나면 그 자리에서 끝나므로 낮은 값도 회귀를 잡는다.

| 환경변수 | 뜻 | 기본 |
|---|---|---|
| `PPWK_E2E_ROUNDS` | 동시성 시나리오 반복 (E2E-4/6/7) | 3 (`-short` 면 1) |
| `PPWK_E2E_SOAK` | E2E-29 의 지속 시간 | 15s |
| `PPWK_E2E_ISSUES` | E2E-30 의 이슈 수 | 200 |

저장소 형식 조합(E2E-25/26)은 그 형식을 지원하지 않는 git 에서 자동으로 skip 한다.
...
단위 테스트로는 잡히지 않았고 E2E 에서 드러난 것들이다. 전부 회귀 테스트를 함께 두었다.

| 결함 | 드러난 곳 | 왜 단위 테스트가 놓쳤나 |
|---|---|---|
| 살아 있는 소유자의 claim 이 회수됨 | E2E-5 (1/6 확률) | 잠금 기록을 잠금 없이 읽어 찢어진 JSON 을 손상으로 오판. 프로세스 8개가 동시에 갱신할 때만 창이 열린다 |
| 훅 없는 사용자가 두 번째 명령을 못 씀 | E2E-22c | 감지 실패 시 명령마다 세션 nonce 가 새로 생겨 `claim` 다음 `start` 가 자기 것이 아니게 된다. 단위 테스트는 세션을 고정해 놓고 돌린다 |
| SHA-256 저장소에서 아무것도 안 됨 | E2E-25 | `update-ref` 에 40자리 zero OID 를 문자열로 넘겼다. SHA-1 저장소에서는 영원히 드러나지 않는다 |
| `list --mine` 이 필터하지 않음 | E2E-1b | 플래그가 선언만 되고 읽히지 않았다. 플래그 배선은 CLI 를 실제로 실행해야 보인다 |
| 알 수 없는 명령이 exit 0 | E2E-10f | `CommandNotFound` 가 메시지만 쓰고 종료 코드를 남기지 않았다. 종료 코드는 프로세스 경계 밖에서만 관측된다 |
| `doctor` 에 stale lock 항목 없음 | E2E-13 | 기능 명세 §1 에는 있는데 구현되지 않았다 |
| reftable 저장소에서 알 수 없는 오류 | E2E-26 | go-git 의 내부 오류가 그대로 새어 나왔다 |
