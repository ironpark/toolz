...
**주의 — 방어가 세 겹이라 순진하게 쓰면 회귀를 못 잡는다.** §4.3 은 trailer, `issue.json` 의 `session` 필드, committer email 세 곳에 세션을 넣는다. 세 겹이 다 살아 있으면 trailer 를 빼도 나머지 둘 때문에 OID 가 여전히 갈려 테스트가 통과한다. T2.3 은 문서와 서명을 고정하고 **trailer 만 변수로 남겨야** 한다. 전체 경로가 갈리는지는 별도 테스트로 본다.
