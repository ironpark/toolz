...
도구가 감지되지 않으면 **부모 프로세스를 세션으로 삼는다** (`shell-<hash of ppid+starttime>`). 명령마다 nonce 를 새로 만들면 세션이 명령 하나짜리가 되어, `claim` 다음의 `start` 가 자기 것이 아니게 되고 `list --mine` 이 언제나 빈다. 같은 셸에서 실행한 명령들은 같은 PPID 를 갖고, 다른 터미널은 다르며, 셸이 죽으면 그 세션도 끝난다 — 세션이라는 말의 뜻 그대로다. 훅 경로가 부모를 도구로 보는 것과 같은 논리다. 부모조차 알 수 없으면 nonce 로 간다.

#### worktree 배타의 판정

세션이 다를 때 막을지는 증거의 종류에 따라 갈린다.

- `hook_pid` 가 있으면 그 세션의 프로세스가 지금 살아 있다는 증거다. 이름이 같아도 (같은 worktree 의 claude-code 두 개) 막는다.
- `hook_pid` 가 없으면 근거는 `last_activity` 뿐이고, 그것은 "최근에 명령이 있었다" 는 뜻일 뿐이다. 이때는 **에이전트 ID 가 다를 때만** 막는다. 에이전트 ID 는 이미 hostname 과 worktree 를 담고 있으므로, 같은 ID 는 같은 사람의 다음 명령으로 본다.
