...
```bash
paperwork doctor      # 환경 점검
paperwork fsck        # 데이터 정합성 점검
```
...
paperwork agents      # 전체 현황
```

작업을 다시 받으세요. 세션은 자동으로 재등록됩니다.

```bash
paperwork next --claim
```
...
paperwork release --mine                  # 대화 종료 시 직접 정리
```
...
cd ../repo-b && paperwork next --claim
```
...
`paperwork` 버전이 저장소 스키마와 맞지 않습니다. 업그레이드가 필요하니 멈추고 보고하세요.

## 후보가 계속 비어 있다

```bash
paperwork list --status open          # open 이슈가 있는지
paperwork next --dry-run              # 왜 후보에서 빠지는지
```
