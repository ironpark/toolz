...
**정정** — 두 문구는 배타적이지 않다. git 은 CAS 실패도 `cannot lock ref '<ref>': is at <a> but expected <b>` 로 감싸서 낸다. 잠금 검사를 먼저 하면 모든 경쟁 패배가 재시도 가능 오류로 둔갑하므로, 분류기는 **CAS 판정을 먼저** 해야 한다 (§14.6).
