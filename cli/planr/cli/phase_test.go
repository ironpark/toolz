...
func TestEnsureDependenciesMetBlocksOutOfOrderPhases(t *testing.T) {
	...
	for _, status := range []string{"in-progress", "done"} {
		err := plan.EnsureDependenciesMet(directories, planRoot, "00-checkout-v2", 1, status)
		if err == nil {
			t.Fatalf("plan.EnsureDependenciesMet(%q) allowed phase 01 while phase 00 was planned", status)
		}
		if !strings.Contains(err.Error(), "API Contract") || !strings.Contains(err.Error(), "--force") {
			t.Fatalf("plan.EnsureDependenciesMet(%q) error = %v, want the blocking phase and --force", status, err)
		}
	}
	// Moving backwards is never blocked.
	if err := plan.EnsureDependenciesMet(directories, planRoot, "00-checkout-v2", 1, "planned"); err != nil {
		t.Fatalf("plan.EnsureDependenciesMet(planned) unexpected error: %v", err)
	}
	// A phase with no unfinished prerequisites proceeds.
	if err := plan.EnsureDependenciesMet(directories, planRoot, "00-checkout-v2", 0, "in-progress"); err != nil {
	...
	if err := plan.EnsureDependenciesMet(directories, planRoot, "00-checkout-v2", 1, "done"); err != nil {
		t.Fatalf("plan.EnsureDependenciesMet() still blocked phase 01 after phase 00 was done: %v", err)
	}
}
...
func TestEnsureDependenciesMetBlocksOnUnfinishedPlans(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "not registered") {
		t.Fatalf("plan.EnsureDependenciesMet() error = %v, want an unregistered dependency", err)
	}
	...
	if err == nil || !strings.Contains(err.Error(), "in-progress") {
		t.Fatalf("plan.EnsureDependenciesMet() error = %v, want an unfinished dependency", err)
	}
...
func TestUpdatePhaseChecklist(t *testing.T) {
	...
	updated, err := plan.UpdateChecklist(body, 0, true)
	if err != nil {
		t.Fatalf("plan.UpdateChecklist() unexpected error: %v", err)
	}
	if !strings.Contains(updated, "- [x] [Phase 00: API Contract]") {
		t.Fatalf("phase 00 was not checked:\n%s", updated)
	}
	updated, err = plan.UpdateChecklist(updated, 1, false)
	if err != nil {
		t.Fatalf("plan.UpdateChecklist() reset unexpected error: %v", err)
	}
...
func TestValidatePhaseStatusChange(t *testing.T) {
	if err := plan.ValidateStatusChange(map[string]any{"entry_condition": nil}, "conditional"); err == nil || !strings.Contains(err.Error(), "entry_condition") {
		t.Fatalf("conditional phase without entry condition error = %v", err)
	}
	if err := plan.ValidateStatusChange(map[string]any{"entry_condition": "only when ready"}, "planned"); err == nil || !strings.Contains(err.Error(), "entry_condition: null") {
		t.Fatalf("planned phase with entry condition error = %v", err)
	}
	if err := plan.ValidateStatusChange(map[string]any{"entry_condition": "only when ready"}, "in-progress"); err != nil {
