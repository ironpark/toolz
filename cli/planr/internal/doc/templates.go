...
func RenderNewDraft(language, name string, dependsOn []string, descriptions ...string) (string, error) {
	...
	if err := draftTemplates.ExecuteTemplate(&output, StringsFor(language).Template, struct {
		Name        string
		Description string
		DependsOn   []string
...
func RenderNewPhaseDraft(language, plan, title, slug string) (string, error) {
	var output bytes.Buffer
	if err := draftTemplates.ExecuteTemplate(&output, StringsFor(language).PhaseTemplate, struct {
		Plan  string
		Title string
		Slug  string
