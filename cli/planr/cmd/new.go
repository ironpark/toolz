...
func newDraftCommand() *ucli.Command {
	return &ucli.Command{
		Name:      "new",
...
func runNew(_ context.Context, cmd *ucli.Command) error {
	...
	if strings.Contains(selector, "#") {
		...
		return runNewPhase(cmd, selector)
	}
	return runNewPlan(cmd)
}

func runNewPlan(cmd *ucli.Command) error {
	name := cmd.Args().First()
	...
	if err := hooks.Run(repoRoot, settings.Hooks, settings.SkipHooks, "before", hooks.EventNew, name, -1, "draft", progressWriter(cmd)); err != nil {
		return err
	}
	...
	if err := hooks.Run(repoRoot, settings.Hooks, settings.SkipHooks, "after", hooks.EventNew, name, -1, "draft", progressWriter(cmd)); err != nil {
		return err
	}
	return nil
}

func runNewPhase(cmd *ucli.Command, selector string) error {
	...
	if err := hooks.Run(repoRoot, settings.Hooks, settings.SkipHooks, "before", hooks.EventNew, planDirectory, -1, "draft", progressWriter(cmd)); err != nil {
		return err
	}
	if cmd.Bool("json") {
	...
	return hooks.Run(repoRoot, settings.Hooks, settings.SkipHooks, "after", hooks.EventNew, planDirectory, -1, "draft", progressWriter(cmd))
}
