<script lang="ts">
    import { ChevronRight, Database, HardDrive, Layers3, Network, Server, Settings, Share2, Users, Stethoscope, ListChecks } from '@lucide/svelte';
    ...
    const icons = { overview: Layers3, storage: HardDrive, datasets: Database, services: Share2, network: Network, identity: Users, system: Server, settings: Settings, diagnostics: Stethoscope, presets: ListChecks } as const;
</script>
...
<aside class="flex h-dvh flex-col bg-sidebar text-sidebar-foreground">
    ...
    <div class="space-y-3 border-t p-3">
        ...
        <Button variant="ghost" class={`w-full justify-start gap-3 max-lg:justify-center ${activeView === 'settings' ? 'bg-sidebar-accent font-medium text-sidebar-accent-foreground' : ''}`} href="/settings" aria-label="앱 설정" aria-current={activeView === 'settings' ? 'page' : undefined}><Settings class="size-4" /><span class="max-lg:hidden">앱 설정</span></Button>
    </div>
</aside>
