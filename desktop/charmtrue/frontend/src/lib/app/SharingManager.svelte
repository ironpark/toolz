...
<section class="space-y-6">
    ...
    <section class="min-w-0 space-y-4"><header class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div><h2 class="text-base font-semibold tracking-tight">파일 공유</h2><p class="mt-1 text-sm text-muted-foreground">{shares.length}개 항목</p></div><div class="flex flex-wrap items-center gap-2"><Input class="min-w-48 flex-1 lg:w-64" bind:value={query} placeholder="이름, 경로, 프로토콜 검색" /><Button variant="outline" size="sm" onclick={() => createShare('NFS')}><Plus />NFS 추가</Button><Button size="sm" onclick={() => createShare('SMB')}><Plus />SMB 추가</Button></div></header><div class="min-w-0">
        {#if shares.length}
